...
using TechLead_SGE.Server.BL.Classes.Data.Actions;
...
namespace TechLead_SGE.Server.BL.Classes.Data.Init
{
    /// <summary>
    /// Clase de configuración inicial para acceder a la Base de Datos.
    /// </summary>
    /// <param name="Dependencies">Objeto de Tipo Dependencies.</param>
    internal class InitData(IDependenciesContext Dependencies)
    {
        /// <summary>
        /// Objeto de tipo ContextDataModel, que permite el manejo de la conexión hacía la BD.
        /// </summary>
        private readonly ContextDataModel ContextData = new(Dependencies);
        ...
        public async Task<List<T>> GetObjects<T>(ParamsDataDto Params)
        {
            ...
            if (Params != null)
            {
                ...
                if (resMethods != null)
                {
                    if (resMethods is List<object> methodsList)
                        listObjects = methodsList.Select(item => ContextData.Dependencies.Mapper!.Map<T>(item)).ToList();
                    else
